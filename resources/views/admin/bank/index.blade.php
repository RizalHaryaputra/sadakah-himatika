                    ...
                    <div class="mb-4">
                        <form action="{{ route('admin.bank.index') }}" method="GET">
                            <div class="flex items-center">
                                <input type="text" name="search" placeholder="Cari berdasarkan nama bank sampah..."
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left font-medium text-gray-600">Jam Operasional</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($lokasiBankSampah as $index => $bank)
                                <tr>
                                    <td class="px-4 py-2">{{$lokasiBankSampah->firstItem() + $index}}</td>
                                    <td class="px-4 py-2">{{$bank->name}}</td>
                                    <td class="px-4 py-2">
                                        <img src="{{ asset('storage/'. $bank->image_url) }}" alt="{{ $bank->name }}"
                                            class="w-10 h-10 object-cover rounded">
                                    </td>
                                    <td class="px-4 py-2">{{$bank->address}}</td>
                                    <td class="px-4 py-2">{{$bank->operation_hours}}</td>
                                    <td class="px-4 py-2">{{$bank->phone_number}}</td>
                                    <td class="px-4 py-2 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{route('admin.bank.edit', $bank)}}" title="Edit"
                                            ...
                                            <form action="{{route('admin.bank.destroy', $bank)}}" method="POST"
                                                id="formDelete">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4 text-gray-500">Bank sampah tidak ditemukan.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
