            ...
            <div class="bg-white overflow-hidden rounded-2xl shadow-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-semibold">Daftar Hadiah</h3>
                        <a href="{{ route('admin.hadiah.create') }}">
                            <div
                    ...
                    <div class="mb-4">
                        <form action="{{ route('admin.hadiah.index') }}" method="GET">
                            <div class="flex items-center">
                                <input type="text" name="search" placeholder="Cari berdasarkan nama hadiah..."
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($hadiahPoin as $index => $hadiah)
                                <tr>
                                    <td class="px-4 py-2">{{$hadiahPoin->firstItem() + $index}}</td>
                                    <td class="px-4 py-2">{{$hadiah->name}}</td>
                                    <td class="px-4 py-2">
                                        <img src="{{ asset('storage/' . $hadiah->image_url) }}"
                                            alt="{{ $hadiah->name }}" class="w-10 h-10 object-cover rounded">
                                    </td>
                                    <td class="px-4 py-2">{{$hadiah->point_cost}} poin</td>
                                    <td class="px-4 py-2">{{$hadiah->stock}}</td>
                                    <td class="px-4 py-2">
                                        <span
                                            class="px-2 py-1 rounded text-xs font-semibold
                                                {{ $hadiah->is_active == true ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            {{ $hadiah->is_active == true ? 'Aktif' : 'Nonaktif' }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-2 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{route('admin.hadiah.edit', $hadiah)}}" title="Edit"
                                            ...
                                            <form action="{{route('admin.hadiah.destroy', $hadiah)}}" method="POST"
                                                id="deleteForm">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4 text-gray-500">Hadiah tidak ditemukan.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
